...
{   
    ...
    ConstMatlabMultiArray<double> weight(prhs[13]);
	...
	for( ;neigh_iter!=neighbors.end();++neigh_iter)
	{
		...
		for( ;set_iter!=set_neigh.end();++set_iter)
		{
			...
			{
				double feature[8];
				feature[0]=x2_difference(full_L_Hist[reg_a],full_L_Hist[reg_b]);
				feature[1]=x2_difference(full_a_Hist[reg_a],full_a_Hist[reg_b]);
				feature[2]=x2_difference(full_b_Hist[reg_a],full_b_Hist[reg_b]);
				feature[3]=x2_difference(full_t_Hist[reg_a],full_t_Hist[reg_b]);
				feature[4]=x2_difference(full_s_Hist[reg_a],full_s_Hist[reg_b]);

				int  area =full_region_area[reg_a]+full_region_area[reg_b];
				feature[5]=1-area/image_area;

				unsigned int bbox[4];
				compute_bbox(full_region_bbox[reg_a],full_region_bbox[reg_b],bbox);
				int bbox_area=(bbox[2]-bbox[0]+1)*(bbox[3]-bbox[1]+1);
				feature[6]=1-(bbox_area-area)/image_area;

				double bdry_strength=bdry_map[reg_a][reg_b].bdry_strength;
				int bdry_length=bdry_map[reg_a][reg_b].bdry_length;
				feature[7]=bdry_strength/bdry_length;
				...
				{
					if(split[ii]!=0)
						split_feature[feature_iter++]=feature[ii];
				}
	...
	while(merge_id<merge_num)
	{
		...
		for( ;set_iter!=reg_c_neigh.end();++set_iter)
		{
			...
			double feature[8];
			feature[0]=x2_difference(full_L_Hist[curr_id],full_L_Hist[neighbor_id]);
			feature[1]=x2_difference(full_a_Hist[curr_id],full_a_Hist[neighbor_id]);
			feature[2]=x2_difference(full_b_Hist[curr_id],full_b_Hist[neighbor_id]);
			feature[3]=x2_difference(full_t_Hist[curr_id],full_t_Hist[neighbor_id]);
			feature[4]=x2_difference(full_s_Hist[curr_id],full_s_Hist[neighbor_id]);

			int area=full_region_area[curr_id]+full_region_area[neighbor_id];
			feature[5]=1-area/image_area;

			compute_bbox(full_region_bbox[curr_id],full_region_bbox[neighbor_id],bbox);
			int bbox_area=(bbox[2]-bbox[0]+1)*(bbox[3]-bbox[1]+1);
			feature[6]=1-(bbox_area-area)/image_area;
